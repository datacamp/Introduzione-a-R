...
I commenti sono ignorati completamente dall'interprete, e quindi non influenzeranno in alcun modo il codice che hai scritto. Ad esempio, _Calculate 3 + 4_  nell'editor a destra è un commento.

Puoi eserguire codice in due modi. Il primo, consiste nello scrivere il programma su un file di testo e successivamente eseguirlo, il secondo modo è la modalità interattiva, infatti è possibile eseguire comandi R immettendoli direttamente nella console.
...
Un elemento base della programmazione si chiama **variabile**.
...
## Assegnazione variabili (3)

Ogni cestino di frutta che si rispetti deve essere un minimo vario, quindi decidiamo di aggiungere sei arance. Come data analyst, la prima cosa che è necessario effettuare è creare la variabile `mie_arance` e assegnarle il valore 6. Successivamente, volendo calcolare quanti pezzi unici di frutta hai in totale, puoi effettuare questa operazione in modo chiaro utilizzando le variabili create precedentemente:
...
La fisica ci dice che non si possono sommare mele e arance. Però l'hai appena fatto, no :-)? Le variabili `mie_mele` e `mie_arance` nell'esercizio precedente contenevano entrambe un numero both contained a number in the previous exercise. L'operatore `+` in R funziona con variabili di tipo numerico (numeric). Se avessi provato a sommare variabili di tipo diverso, per esempio assegnando a `mie_arance` una variabile di tipo character (vedi nell'editor editor), effettuando la somma proveresti ad assegnare alla variabile `mia_frutta` una somma tra una variabile di tipo character e una di tipo numerico. Questa operazione, ovviamente, non è possibile.
...
*** =sct
```{r}
test_error(incorrect_msg = "You can do this by setting the `mie_arance` variable to a numeric value, not a string!")
