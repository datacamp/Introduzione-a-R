...
- 'Sei spostato/a?' oppure altre domande a risposta chiusa 'si/no' (`logical`)
...
*** =instructions
Clicca 'Submit Answer'. Il contenuto del data frame di esempio [`mtcars`](http://www.rdocumentation.org/packages/datasets/functions/mtcars) sarà stampato a video nella console.
...
Lavorare con grandi data sets è molto comune nell'analisi dati. Quando lavori con data sets estremamente grandi, la tua priorità numero uno è comprendere la sua struttura e gli elementi principali. Per questo motivo, è spesso utile stampare a video una frazione dell'intero data set.

Come puoi svolgere questa operazione in R? La funzione [`head()`](http://www.rdocumentation.org/packages/utils/functions/head) ti permette di visualizzare le prime osservazioni di un data frame. In modo duale, la funzione [`tail()`](http://www.rdocumentation.org/packages/utils/functions/head) stampa le ultime osservazioni contenute nel tuo data set.

Sia [`head()`](http://www.rdocumentation.org/packages/utils/functions/head) che [`tail()`](http://www.rdocumentation.org/packages/utils/functions/head) stampano una riga chiamata 'header', che contiene i nomi delle variabili contenute nel tuo data set.
...
--- typeNormalExercise xp:100 skills:1
## Creare un data frame

Siccome utilizzare data sets disponibili di default non è molto divertente, il resto del capitolo è focalizzato su un data set creato da te. Allacciati il ket pack siccome è tempo di esplorare lo spazio!
...
--- tipo:NormalExercise xp:100 skills:1
...
In modo simile ai vettori e alle matrici, puoi selezionare elementi da un data frame utilizzando le parentesi quadre `[ ]`. Utilizzando una virgola, puoi indicare cosa selezionare, rispettivamente, dalle righe e dalle colonne. Per esempio:
...
Speeso ti capiterà di voler scegliere una colonna intera, ossia una variabile specifica, dal tuo data frame. Se vuoi selezionare tutti gli elementi della variabile `diametro`, ad esempio, le seguenti linee di codice ti permetteranno entrambe di raggiungere l'obiettivo:
...
Quindi, cosa hai imparato negli esercizi precedenti? Hai selezionato un sottoinsieme di dati da un data frame (`pianeti_df`) sulla base di una certa condizione (pianeta con anelli oppure senza), e sei riuscito a recuperare tutti i dati relativi alle osservazioni interessate. Direi che sei a buon punto! La NASA sta probabilmente già flirtando con il tuo CV ;-). 
...
[`order()`](http://www.rdocumentation.org/packages/base/functions/order) è una funzione che ti ritorna l'ordine, ossia il modo in cui gli elementi del vettore passato come argomento dovrebbero essere disposti affinchè siano disposti in ordine crescente. Un esempio chiarirà meglio la nebulosa frase che ho appena scritto:
...
Questo significa che possiamo utilizzare l'output di `order(a)` per riordinare il vettore `a`:
...
Bene, ora che conosci la funzione [`order()`](http://www.rdocumentation.org/packages/base/functions/order), utilizziamola per qualcosa di utile. Supponi di voler riordinare il tuo dataframe cosi che inizi con il pianeta più piccolo e finisca con quello più grande. A sort on the `diametro` column.

*** =instructions
- Chiama `order()` su `pianeti_df$diametro` (la colonna `diametro` di `pianeti_df`). Salva il risultato come `posizioni`.
- Ora sistema `pianeti_df` utilizzando il `posizioni` come vettore degli indici di riga dentro le parentesi quadrate. Tieni tutte le colonne e poi stampa il risultato.

*** =hint
- Usa `order(pianeti_df$diametro)` per creare `posizioni`.
- Ora puoi utilizzare `posizioni` dentro le parentesi quadre: `pianeti_df[...]`; Riesci a riempier i puntini `...`?
