...
title       : Matrici
description : In questo capitolo imparerai come utilizzare le matrici in R. Alla fine del capitolo, avrai appreso come creare matrici e come effettuare le operazioni di base tra matrici. Per capire come usare le matrici, inizierai analizzando i risultati di Star Wars al box office! Che la forza sia con te!

--- type:NormalExercise xp:100 skills:1 key:e47ee5cd8d
## Che cos'è una matrice?

In R, una matrice è una collezione di elementi dello stipo tipo (numeric, character o logico) disposti in modo ordinato su un numero fisso di righe e colonne. Siccome in questo tutorial lavorerai solo con righe e colonne, queste matrici con cui lavorerai sono bidimensionali.
...
Tempo di iniziare a sporcarsi le mani con le matrici! Nel prossimo esercizio analizzerai i risultati del box office di Star Wars. Che la forza sia con te!

Nell'editor sono stati definiti tre vettori. Ognuno rappresenta i numeri del box office dei primi tre film di Star Wars. Il primo elemento di ciascun vettore indica i risultati al box office negli Stati Uniti, il secondo elemento si riferisce ai risultati del resto del mondo (fonte: Wikipedia).
...
- Costruisci una matrice con 3 righe, dove ogni riga rappresenta uno dei tre film. Per fare questo, utilizza la funzione `matrix()`. Il primo argomento è il vettore `box_office`, contenente tutti i risultati del box office figures. Successivamente dovrai specificare il numero di righe, `nrow = 3`, e che vuoi riempire la matrice per riga, `byrow = TRUE`. Chiama la matrice che otterrai `matrice_star_wars`.
...
Per aiutarti a ricordare cosa è stato immagazzinato alla matrice `matrice_star_wars`, sarebbe utile poter aggiungere i nomi dei film alle righe. Questa operazione ti aiuterebbe non solo a leggere meglio i dati ma anche selezionare solo certi elementi dalla matrice.
...
- Stampa a video `matrice_star_wars` per vedere il risultato del tuo lavoro.
...
test_output_contains("matrice_star_wars", incorrect_msg = "Don't forget to print out `matrice_star_wars` after you've named the rows and columns.")
success_msg("Molto bene! Sei sulla buona strada per diventare uno jedi di R!You're on the way of becoming an R jedi! Continua verso l'esercizio successivo!")
```
...
Per calcolare il risultato coplessivo al box office, devi fare la somma dei ricavi ottenuti negli Stati Uniti e nel resto del mondo.
...
*** =instructions
Aggiungi il vettore `risultato_mondo` come nuova colonna alla matrice `matrice_star_wars` e assegnail risultato complessivo alla matrice `tutti_star_wars_matrice`. Utilizza la funzione [`cbind()`](http://www.rdocumentation.org/packages/base/functions/cbind).

*** =hint
In questo esercizio, devi passare due variabili a `cbind()`: `matrice_star_wars` e `risultato_mondo`, in questo preciso ordine. Assegna il risultato a `tutti_star_wars_matrice`.
...
# Unisci per colonna la nuova variabile risultato_mondo alla matrice matrice_star_wars
tutti_star_wars_matrice <- cbind(matrice_star_wars, risultato_mondo)
```
...
msg <- "Hai usato `cbind()` per aggiungere `risultato_mondo` a `matrice_star_wars`? Dovresti passare `matrice_star_wars` e `world_wide_vector` a `cbind()`, in questo ordine. La matrice risultante, `tutti_star_wars_matrice`, dovrebbe avere tre righe e tre colonne."
test_object("tutti_star_wars_matrice", eval = FALSE)
test_what(
  expect_equal(
    dim(get("tutti_star_wars_matrice", get_solution_env())), 
    dim(get("tutti_star_wars_matrice", .GlobalEnv))),
  feedback = msg)

test_object("tutti_star_wars_matrice", incorrect_msg = msg)
...
*** =instructions
Utilizza `rbind()` per unire per riga `matrice_star_wars` e `matrice_star_wars2`, in questo preciso ordine. Assegna il risultato alla matrice `tutti_star_wars_matrice`.
...
```
Assegna il risultato a `tutti_star_wars_matrice`.
...
# Rimuovi tutto tranne tutti_star_wars_matrice
rm(box_office_all)
rm(movie_names)
rm(col_titoli)
rm(box_office_all2)
...
# Combina entrambe le matrici dei film di Star Wars in una matrice
tutti_star_wars_matrice <- rbind(matrice_star_wars, matrice_star_wars2)
```
...
test_object("tutti_star_wars_matrice", incorrect_msg = "Hai utilizzato `rbind()` in modo corretto per creare `tutti_star_wars_matrice()`? `rbind()` dovrebbe prendere due argomenti: `matrice_star_wars` e `matrice_star_wars2`, in questo ordine.")

success_msg("Fantastico! Continua con l'esercizio successivo e scopri come puoi combinare i risultati della funzione `rbind()` con la funzione `colSums()`!")
```
...
- Stampa a video `vettore_ricavi_totali` per ispezionare il risultato ottenuto.
...
*** =pre_exercise_code
```{r}
load(url("http://s3.amazonaws.com/assets.datacamp.com/course/intro_to_r/tutti_star_wars_matrice.RData"))
```

*** =sample_code
```{r}
# tutti_star_wars_matrice è disponibile nel tuo workspace
tutti_star_wars_matrice
...
# Ricavi totali per Stati Uniti e resto del mondo
vettore_ricavi_totali <- colSums(tutti_star_wars_matrice)
...
*** =sct
```{r}
msg = "Non modificare il contenuto di `tutti_star_wars_matrice`; è stata già caricata per te nel workspace."
test_object("tutti_star_wars_matrice", eq_condition = "equal", undefined_msg = msg, incorrect_msg = msg)
test_function("colSums", "x", incorrect_msg = "Hai utilizzato la funzione `colSums()` sulla matrice tutti_star_wars_matrice?")
test_object("vettore_ricavi_totali",
            incorrect_msg = "Hai assegnato il risultato di `colSums(tutti_star_wars_matrice)` a `vettore_ricavi_totali`?")
...
- `mia_matrice[1:3,2:4]` ritorna una matrice con i dati presenti sulle righe rows 1, 2, 3 e colunne 2, 3, 4.
...
Torniamo a Star Wars con le conoscenze appena acquisite! Come nell'esercizio precedente, `tutti_star_wars_matrice` è già disponibile nel workspace.

*** =instructions
- Seleziona i ricavi ottenuti nel resto del mondo per tutti i film (tutta la seconda colonna di `tutti_star_wars_matrice`), salva il risultato come `tutti_non_usa`.
...
- Questa volta, seleziona i ricavi nel resto del mondo per i primi due film in `tutti_star_wars_matrice`. Salva il risultato come `alcuni_non_usa`.
...
*** =sample_code
```{r}
# tutti_star_wars_matrice è disponibile nel workspace
tutti_star_wars_matrice
...
# Seleziona i ricavi ottenuti nel resto del mondo per tutti i film
tutti_non_usa <- tutti_star_wars_matrice[,2]
  
# Media ricavi nel resto del mondo
mean(tutti_non_usa)
  
# Seleziona i ricavi ottenuti nel resto del mondo per i primi due film
alcuni_non_usa <- tutti_star_wars_matrice[1:2,2]
...
*** =sct
```{r}
msg = "Non cambiare il contenuto di `tutti_star_wars_matrice`; questa matrice è già stata caricata per te nel workspace."
test_object("tutti_star_wars_matrice", undefined_msg = msg, incorrect_msg = msg)

test_object("tutti_non_usa",
            incorrect_msg = "Hai assegnato a `tutti_non_usa` la seconda colonna di `tutti_star_wars_matrice`? Per farlo puoi utilizzare `[, 2]`!")
...
In qualità di data analyst appena assunto da Lucasfilm, è compito tuo capire quante persone hanno visto il film in ogni area geografica. Hai già i ricavi totali nella matrice `tutti_star_wars_matrice`. Assumi che il costo di un biglietto fosse 5 dollari. Dividi i numeri del box office per il costo del biglietto e ottieni il numero di spettatori.

*** =instructions
- Dividi `tutti_star_wars_matrice` per 5, ottenendo il numero di spettatori in milioni. Assegna la matrice risultato alla variabile `spettatori`.
- Stampa `spettatori` così da poter ispezionare il suo contenuto.

*** =hint
Il numero di spettatori è dato da `tutti_star_wars_matrice` diviso per 5.
...
# Stima gli spettatori
spettatori <- tutti_star_wars_matrice / 5
...
test_object("spettatori",
            incorrect_msg = "Sembra che `spettatori` non sia stato calcolato correttamento. Dividi `tutti_star_wars_matrice` per 5 e salva la matrice risultante come `spettatori`.")
...
Così come `2 * mia_matrice` multiplicava ogni elemento di `mia_matrice` per due, `mia_matrice1 * mia_matrice2` crea una matrice dove ogni elemento è il prodotto degli elementi che occupano la stessa posizione in `mia_matrice1` e `mia_matrice2`. 
...
*** =instructions
- Dividi `tutti_star_wars_matrice` per `matrice_prezzo_biglietto` per ottenera la stima del numero di spettatori statunitensi e non per tutti i sei film. Assegna il risultato alla variabile `spettatori`.
...
- Calcola il numero media di spettatori statunitensi; stampa a video il risultato.
...
*** =sample_code
```{r}
# tutti_star_wars_matrice e matrice_prezzo_biglietto sono già disponibili nel workspace
tutti_star_wars_matrice
...
# Numero stimato di spettatori
spettatori <- tutti_star_wars_matrice / matrice_prezzo_biglietto
...
test_object("spettatori",
            incorrect_msg = "Hai creato la matrice `spettatori`? Dovresti dividere `tutti_star_wars_matrice` per `matrice_prezzo_biglietto` per ottenerla.")
