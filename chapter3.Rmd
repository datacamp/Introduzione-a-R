...
*** =instructions
Aggiungi il vettore `risultato_mondo` come nuova colonna alla matrice `matrice_star_wars` e assegna il risultato complessivo alla matrice `all_star_wars_matrix`. Utilizza la funzione [`cbind()`](http://www.rdocumentation.org/packages/base/functions/cbind).

*** =hint
In questo esercizio, devi passare due variabili a `cbind()`: `matrice_star_wars` e `risultato_mondo`, in questo preciso ordine. Assegna il risultato a `all_star_wars_matrix`.
...
# Unisci per colonna la nuova variabile risultato_mondo alla matrice matrice_star_wars
all_star_wars_matrix <- cbind(matrice_star_wars, risultato_mondo)
```
...
msg <- "Hai usato `cbind()` per aggiungere `risultato_mondo` a `matrice_star_wars`? Dovresti passare `matrice_star_wars` e `world_wide_vector` a `cbind()`, in questo ordine. La matrice risultante, `all_star_wars_matrix`, dovrebbe avere tre righe e tre colonne."
test_object("all_star_wars_matrix", eval = FALSE)
test_what(
  expect_equal(
    dim(get("all_star_wars_matrix", get_solution_env())), 
    dim(get("all_star_wars_matrix", .GlobalEnv))),
  feedback = msg)

test_object("all_star_wars_matrix", incorrect_msg = msg)
...
*** =instructions
Utilizza `rbind()` per unire per riga `matrice_star_wars` e `matrice_star_wars2`, in questo preciso ordine. Assegna il risultato alla matrice `all_star_wars_matrix`.
...
```
Assegna il risultato a `all_star_wars_matrix`.
...
# Rimuovi tutto tranne all_star_wars_matrix
rm(box_office_all)
rm(movie_names)
rm(col_titoli)
rm(box_office_all2)
...
# Combina entrambe le matrici dei film di Star Wars in una matrice
all_star_wars_matrix <- rbind(matrice_star_wars, matrice_star_wars2)
```
...
test_object("all_star_wars_matrix", incorrect_msg = "Hai utilizzato `rbind()` in modo corretto per creare `all_star_wars_matrix()`? `rbind()` dovrebbe prendere due argomenti: `matrice_star_wars` e `matrice_star_wars2`, in questo ordine.")
...
*** =pre_exercise_code
```{r}
load(url("http://s3.amazonaws.com/assets.datacamp.com/course/intro_to_r/all_star_wars_matrix.RData"))
```

*** =sample_code
```{r}
# all_star_wars_matrix è disponibile nel tuo workspace
all_star_wars_matrix
...
# Ricavi totali per Stati Uniti e resto del mondo
vettore_ricavi_totali <- colSums(all_star_wars_matrix)
...
*** =sct
```{r}
msg = "Non modificare il contenuto di `all_star_wars_matrix`; è stata già caricata per te nel workspace."
test_object("all_star_wars_matrix", eq_condition = "equal", undefined_msg = msg, incorrect_msg = msg)
test_function("colSums", "x", incorrect_msg = "Hai utilizzato la funzione `colSums()` sulla matrice all_star_wars_matrix?")
test_object("vettore_ricavi_totali",
            incorrect_msg = "Hai assegnato il risultato di `colSums(all_star_wars_matrix)` a `vettore_ricavi_totali`?")
...
Torniamo a Star Wars con le conoscenze appena acquisite! Come nell'esercizio precedente, `all_star_wars_matrix` è già disponibile nel workspace.

*** =instructions
- Seleziona i ricavi ottenuti nel resto del mondo per tutti i film (tutta la seconda colonna di `all_star_wars_matrix`), salva il risultato come `tutti_non_usa`.
...
- Questa volta, seleziona i ricavi nel resto del mondo per i primi due film in `all_star_wars_matrix`. Salva il risultato come `alcuni_non_usa`.
...
*** =sample_code
```{r}
# all_star_wars_matrix è disponibile nel workspace
all_star_wars_matrix
...
# Seleziona i ricavi ottenuti nel resto del mondo per tutti i film
tutti_non_usa <- all_star_wars_matrix[,2]
  
# Media ricavi nel resto del mondo
mean(tutti_non_usa)
  
# Seleziona i ricavi ottenuti nel resto del mondo per i primi due film
alcuni_non_usa <- all_star_wars_matrix[1:2,2]
...
*** =sct
```{r}
msg = "Non cambiare il contenuto di `all_star_wars_matrix`; questa matrice è già stata caricata per te nel workspace."
test_object("all_star_wars_matrix", undefined_msg = msg, incorrect_msg = msg)

test_object("tutti_non_usa",
            incorrect_msg = "Hai assegnato a `tutti_non_usa` la seconda colonna di `all_star_wars_matrix`? Per farlo puoi utilizzare `[, 2]`!")
...
In qualità di data analyst appena assunto da Lucasfilm, è compito tuo capire quante persone hanno visto il film in ogni area geografica. Hai già i ricavi totali nella matrice `all_star_wars_matrix`. Assumi che il costo di un biglietto fosse 5 dollari. Dividi i numeri del box office per il costo del biglietto e ottieni il numero di spettatori.

*** =instructions
- Dividi `all_star_wars_matrix` per 5, ottenendo il numero di spettatori in milioni. Assegna la matrice risultato alla variabile `spettatori`.
- Stampa `spettatori` così da poter ispezionare il suo contenuto.

*** =hint
Il numero di spettatori è dato da `all_star_wars_matrix` diviso per 5.
...
# Stima gli spettatori
spettatori <- all_star_wars_matrix / 5
...
test_object("spettatori",
            incorrect_msg = "Sembra che `spettatori` non sia stato calcolato correttamento. Dividi `all_star_wars_matrix` per 5 e salva la matrice risultante come `spettatori`.")
...
*** =instructions
- Dividi `all_star_wars_matrix` per `matrice_prezzo_biglietto` per ottenera la stima del numero di spettatori statunitensi e non per tutti i sei film. Assegna il risultato alla variabile `spettatori`.
...
*** =sample_code
```{r}
# all_star_wars_matrix e matrice_prezzo_biglietto sono già disponibili nel workspace
all_star_wars_matrix
...
# Numero stimato di spettatori
spettatori <- all_star_wars_matrix / matrice_prezzo_biglietto
...
test_object("spettatori",
            incorrect_msg = "Hai creato la matrice `spettatori`? Dovresti dividere `all_star_wars_matrix` per `matrice_prezzo_biglietto` per ottenerla.")
