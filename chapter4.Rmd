...
Il termine fattore si riferisce ad un particolare tipo di dato statistico utilizzato per immagazzinare variabili di tipo categorico. La differenza tra una variabile di tipo categorico e una variabile di tipo continuo consiste nel fatto che la variabile categorica può appartenere ad un **numero di categorie limitato**. Una variabile di tipo continuo, al contrario, può assumere infiniti valori.

Permettere a R di sapere se sta operando su variabili di tipo categorico o continuo è molto imporante siccome i modelli statistici che utilizzerai in futuro trattano i due tipi di variabile in modo molto diverso. (In futuro ti sarà spiegato il perchè).

Un buon esempio di una variabile categorica è il genere. Una persona può essere o "Maschio" o "Femmina". In questo caso, "Maschio" e "Femmina" sono i due possibili valori che la variabile categorica "genere" può assumere e ad ogni unità statistica sarà assegnato uno dei due possibili valori.

*** =instructions
Assegna lla variabile `teoria` il valore `"fattori per variabili categoriche"`.
...
*** =instructions
- Converti il vettore di character `vettore_genere` in un vettore di fattori utilizzando `factor()` e assegna il risultato a `fattore_vettore_genere`
...
```{r}
# Vettore genere
vettore_genere <- c("Male", "Female", "Female", "Male", "Male")
...
*** =sct
```{r}
test_object("factor_vettore_genere",
...
success_msg("Riesci ad intuire il risultato dell'operazione effettuata in questo esercizio? Eccezionale! Continua con il prossimo esercizio e studia in dettaglio i livelli dei fattori.")
```
...
- Cambia i livelli di `factor_vettore_sondaggio` da quelli attuali a `c("Female", "Male")`. Fai attenzione all'ordine degli elementi del vettore.
...
test_object("factor_vettore_sondaggio", eq_condition = "equal",
            incorrect_msg = paste("Hai assegnato correttamente i livelli a `factor_vettore_sondaggio`? Utilizza `levels(factor_vettore_sondaggio) <- c(\"Female\", \"Male\")`. Ricorda che R è case sensitive!"))
...
Tornando al nostro sondaggio, vorresti sapere quante risposte hai ottenuto da ciascun sesso. La funzione [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary) ti permette di ottenere la risposta a questa domanda.
...
In `factor_vettore_sondaggio` abbiamo un fattore con due livelli: Maschio e Femmina. Ma in che modo R valuta questi livelli tra di loro? In altre parole, quale è il migliore dei due, secondo R?

*** =instructions
Leggi il codice nell'editore e clicca 'Submit Answer' per vedere quale dei due sessi R preferisce.

*** =hint
Clicca semplicemente 'Submit Answer' e osserva l'output che viene stampato a video nella console.
...
Tuttavia non è sempre così! Alcune volte avrai a che fare con fattori i cui livelli presentano un ordinamento naturale. In questo caso, dobbiamo assicurarci di passsare l'informazione sull'ordinamento a R.

Diciamo che sei a capo di un gruppo di ricerca composto da 5 analisti e che vuoi valutare le loro performances. Per fare la tua valutazione, prendi nota della loro velocità e classifichi ogni analista come `"lento"`, `"veloce"` o `"velocissimo"`, e salva i risultati in `vettore_velocita`.
...
- Analyst 2 è lento, 
- Analyst 3 è lento,
- Analyst 4 è veloce e
- Analyst 5 è velocissimo.

Non è necessario specificare che si tratta di fattori al momento.

*** =hint
Assegna a `vettore_velocita` un vettore contenente le stringhe `"fast"`, `"slow"` ...
...
```{r}
# Crea vettore_velocita
vettore_velocita <- c("fast", "sBassa", "sBassa", "fast", "insane")
```
...
*** =instructions
Da `vettore_velocita`, crea un fattore ordinale: `factor_vettore_velocita`. Fissa `ordered` a `TRUE`, e fissa `levels` a `c("sBassa", "fast", "insane")`.
...
test_function("factor", "levels", incorrect_msg = "Assicurati di fissare `levels = c(\"sBassa\", \"fast\", \"insane\")` nella chiamata alla funzione `factor()`.")
test_object("factor_vettore_velocita", eq_condition = "equal",
            incorrect_msg = "C'è ancora qualcosa di errato in `factor_vettore_velocita`; assicurati di passare solo `vettore_velocita`, `ordered = TRUE` and `levels = c(\"sBassa\", \"fast\", \"insane\")` dentro la chiamata alla funzione `factor()`.")
