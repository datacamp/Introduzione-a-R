...
- **Data frames** (two-dimensional objects): possono contenere valori di tipo numeric, character o logical. Ogni colonna deve contenere valori dello stesso tipo ma colonne differenti possono contenere valori di tipo differente.
...
In R una lista ti permette di raccogliere una serie di oggetti sotto un "singolo nome" (ossia la lista) in modo ordinato. Questi oggetti possono essere matrici, vettori, data frames, altre liste, ecc... Non è nemmeno richiesto che questi oggetti siano in qualche modo collegati l'uno all'altro.
...
*** =instructions
Costruisci una lista chiamata `mia_lista`, that contains the variables `mio_vettore`, `mia_matrice` and `mio_df` as list components.

*** =hint
Usa la funzione [`list()`](http://www.rdocumentation.org/packages/base/functions/list) con `mio_vettore`, `mia_matrice` e `mio_df` come argomenti, separati da una virgola.
...
Questa operazione creerà una lista con 2 elementi il cui nome sarà `nome1` e `nome2`. Se vuoi assegnare nomi agli elementi di una lista dopo averla definita, puoi usare la funzione `names()` come avevi fatto in precedenza con i vettori. Il codice seguente è equivalenti al codice scritto sopra:
...
```
Aggiungi gli altri due componenti nello stesso modo.
...
Inizia a creare una lista per il film "The Shining". Abbiamo già creato le variabili `film`, `attori` and `rev` nel workspace. Controlla il contenuto di queste variabili nella console R.
...
*** =hint
`shining_list <- list(nome_film = film)` è solo una parte della soluzione; tocca a te inserire anche `attori` e `rev` nella lista con i nomi opportuni.
...
Un modo veloce per verificare che la seleziona sia corretta è digitare il codice nella console. Attenzione: per selezionare elementi dai vettori utilizzavi le parentesi quadre singole `[ ]`. Attento a non fare confusione!
...
Se vuoi assegnare un nome al nuovo elemento, puoi assegnare il nome in modo molto semplice, come abbiamo fatto prima:
